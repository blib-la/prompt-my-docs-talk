...
function v(t, e) {
	...
	let i = 0;
	if (
		(n[0].trim() || n.shift(), n.length > 0 && !n[n.length - 1].trim() && n.pop(), n.length > e)
	)
		n.splice(e);
...
class A {
		...
		if (e) {
			...
			return (
				this.options.sanitize &&
					((t.type = "paragraph"),
					(t.text = this.options.sanitizer ? this.options.sanitizer(e[0]) : o(e[0])),
					(t.tokens = []),
	...
	link(t) {
		...
		if (e) {
			...
			return (
				(n = n.trim()),
				/^</.test(n) &&
					(n = this.options.pedantic && !/>$/.test(t) ? n.slice(1) : n.slice(1, -1)),
				z(
					e,
					{
	...
	emStrong(t, e, n = "") {
		...
		if (!s || (s && ("" === n || this.rules.inline.punctuation.exec(n)))) {
			...
			for (o.lastIndex = 0, e = e.slice(-1 * t.length + n); null != (i = o.exec(e)); ) {
	...
	url(t, e) {
		...
		if ((n = this.rules.inline.url.exec(t))) {
			let t, i;
			if ("@" === n[2]) (t = o(this.options.mangle ? e(n[0]) : n[0])), (i = "mailto:" + t);
			else {
				let e;
				do {
...
(E.def = d(E.def).replace("label", E._label).replace("title", E._title).getRegex()),
	(E.bullet = /(?:[*+-]|\d{1,9}[.)])/),
...
($._punctuation = "!\"#$%&'()+\\-.,/:;<=>?@\\[\\]`^{|}~"),
	...
	($.link = d($.link)
		...
		.getRegex()),
	($.reflink = d($.reflink).replace("label", $._label).replace("ref", E._label).getRegex()),
...
class I {
	constructor(t) {
		...
		this.options.pedantic
			...
			: this.options.gfm &&
			  ((n.block = E.gfm), this.options.breaks ? (n.inline = $.breaks) : (n.inline = $.gfm)),
	...
	inlineTokens(t, e = []) {
		...
		if (this.tokens.links) {
			...
			if (t.length > 0)
				for (; null != (r = this.tokenizer.rules.inline.reflinkSearch.exec(o)); )
				...
				switch (d.type) {
					...
					case "table":
						for (c = "", o = "", r = d.header.length, i = 0; i < r; i++)
							o += this.renderer.tablecell(this.parseInline(d.header[i].tokens), {
								header: !0,
								align: d.align[i],
							});
						for (
...
function P(t, e, n) {
	...
	if (("function" == typeof e && ((n = e), (e = null)), T((e = y({}, P.defaults, e || {}))), n)) {
		const i = e.highlight;
		let s;
		try {
			s = I.lex(t, e);
